class Link < ActiveRecord::Base
  ...
  validates_length_of :url, within: 3..1000, on: :create, message: "too long"
  ...
  # the API
  def self.shorten(url, id = '', slug = '')
    link = Link.where(file_id:  id).first
    return link.short if link

    link = Link.where(url:  url).first
    return link.short if link

    link = Link.new(url: url, file_id: id, slug: slug)
